...
table1_df <- dental_df1 %>%
  ...
  mutate(
    ...
    time_to_dental = if_else(is.infinite(time_to_dental), NA_real_, time_to_dental),
    dental_contact_category = factor(case_when(
      is.na(time_to_dental) ~ "No dental contact",
      time_to_dental < 1 ~ "Dental contact within 24 hours",
      time_to_dental < 7 ~ "Dental contact within 7 days",
      time_to_dental >= 7 ~ "Dental contact after 7 days"
  ...
  mutate(
    ...
    `UDA banding` = factor(TREATMENT_CHARGE_BAND, levels = c("Urgent Treatment", "Band 1", "Band 2", "Band 3", "Free"))
  ) %>% select(
    -index_call_date, 
    -index_call_ref, 
    -dental_date, 
...
## Summary of dental contacts

```{r}
#| label: tbl-one
#| tbl-cap: Summary table of dental contacts

table1_df %>% 
  select(age_bin, sex, ethnicity, IMD_decile, IUC_service_referral, dental_contact_category, `Time to dental contact (days)`, EXEMPTION_REMISSION_FLAG, RADIOGRAPH_FLAG, ANTIBIOTICS, `Dental surgery type`, `UDA banding`,  -dental_contact, -age) %>%
  rename(
    `Age (years)` = age_bin,
...
abs_time_map_fn <- function(df, 
  ...
  if(dental_contact_cat == "Dental contact within 24 hours") {
      ...
      df1 = df %>% 
        ...
        ungroup()
  } else if (dental_contact_cat == "Dental contact after 7 days") {
      df1 = df %>% 
        filter(dental_contact_category == dental_contact_cat) %>%
        group_by(index_call_ref) %>%
        arrange(timestamp) %>%
