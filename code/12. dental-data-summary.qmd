...
dental_time_lu <- dental_df %>% 
  ...
  ) %>% select(index_call_ref, dental_contact_category)#
...
table1_df %>% 
  select(age_bin, sex, ethnicity, IMD_decile, IUC_service_referral, `Time to dental contact (days)`, `Dental contact within 24 hours`, `Dental contact within 7 days`, everything(), -dental_contact, -age) %>%
  rename(
    `Age (years)` = age_bin,
...
imd_df %>%
  select(-index_call_ref, -index_call_date, -dental_contact_category, -ethnicity) %>%
  group_by(IMD) %>%
  summarise(
    n = n(),
...
imd_df %>%
  select(-index_call_ref, -index_call_date, -ethnicity) %>%
  na.omit() %>%
...
imd_df %>%
  select(-index_call_ref, -index_call_date, -IMD) %>%
  mutate(
